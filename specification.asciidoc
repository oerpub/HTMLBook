...
Related but tangential content. HTML5 is kind of confusing on the use of asides. See http://html5doctor.com/aside-revisited/  It would be nice to use the +<aside>+ element for all of these and use 
++data-type++ to distinguish between them. This is slightly different from what HTMLBook does, but incorporates their use of 
++sidebar++. 
...
*Attribute requirements*: ++id++,
 ++data-type="note
 ,tip,warning,important,aside,sidebar"++
 
*Optional attributes*: For compatibility with CNXML: ++data-subtype++ (deprecated, use class instead) User supplied type for compatibility with CNXML
 ++data-display="inline,block,none"++
